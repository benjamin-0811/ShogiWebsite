...
namespace Blank.Shogi {
    internal class Board {
        ...
        internal void InitPieces() {
            BC.Action("Setup all pieces on the board");
            // Pawn
            for (int i = 0; i < 9; i++) {
            ...
            // Silver General
            squares[2, 0].piece = new SilverGeneral(player2, squares[2, 0]);
            squares[2, 8].piece = new SilverGeneral(player1, squares[2, 8]);
            squares[6, 0].piece = new SilverGeneral(player2, squares[6, 0]);
            squares[6, 8].piece = new SilverGeneral(player1, squares[6, 8]);
            // Gold General
            squares[3, 0].piece = new GoldGeneral(player2, squares[3, 0]);
            squares[3, 8].piece = new GoldGeneral(player1, squares[3, 8]);
            squares[5, 0].piece = new GoldGeneral(player2, squares[5, 0]);
            squares[5, 8].piece = new GoldGeneral(player1, squares[5, 8]);
            // King
        ...
        internal void InitCheckMate() {
            // player1
            squares[3, 8].piece = new GoldGeneral(player1, squares[3, 8]);
            squares[4, 7].piece = new GoldGeneral(player1, squares[4, 7]);
            squares[4, 8].piece = new King(player1, squares[4, 8]);
            // player2
        ...
        internal string ToHtml(bool isOver) {
            ...
            for (int i = 0; i < 9; i++) {
                string subText = "";
                for (int j = 0; j < 9; j++) {
                    subText += squares[j, i].ToHtml(isOver);
                }
        ...
        internal string PromotionZone() {
            return $"var promotionZone = {(isPlayer1Turn ? "['a', 'b', 'c']" : "['g', 'h', 'i']")}";
        }

        internal string ForcePawnLancePromotion() {
            return $"var pawnLancePromo = {(isPlayer1Turn ? "'a'" : "'i'")}";
        }

        internal string ForceKnightPromotion() {
            return $"var knightPromo = {(isPlayer1Turn ? "['a', 'b']" : "['h', 'i']")}";
        }
