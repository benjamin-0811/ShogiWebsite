...
{
    internal TX X { get; set; }
    internal TY Y { get; set; }
...
internal class Helper
{
    ...
    internal static byte[] GetFileContentBytes(string path)
    {
        string stylesheet = File.ReadAllText(path);
...
internal class CssBuilder : AbstractBuilder<string>
{
    ...
    {
        bool noSelectors = selectors.Count <= 0;
        bool noStyles = styles.Count <= 0;
...
internal class HtmlBuilder : AbstractBuilder<string>
{
    ...
    private void AddEndTag(StringBuilder builder) => builder.Append($"</{tag}>");
    ...
    private void AddProperties(StringBuilder builder)
    {
        foreach (KeyValuePair<string, string?> property in properties)
            builder.Append($" {property.Key}{(property.Value != null ? $"=\"{property.Value}\"" : "")}");
    }
...
internal class Color
{
    ...
    private void CalcRgba() => rgba = BitConverter.ToInt32(new byte[] { a, b, g, r });
